...
Two, both in act 2 — territory nothing could reach until buffed runs started winning, and
which `--enter-acts` now puts a few seconds away instead of behind a won boss fight.
...
| O13 | `player.relics` step 10: the game takes Pael's Horn, the emulator offers nothing | `act2-jump` | **An act's ancient offers blessings and none of them are modelled.** Hive's three are Orobas, Pael and Tezcatara, one of which each act draws in the LAST line of `ActModel.GenerateRooms` — a draw the emulator spends and throws away. Each offers THREE options off its own `base.Rng`, one per pool, and the pools are deck-conditional: Pael adds `PaelsClaw` when three cards can take a Goopy enchantment and `PaelsTooth` when five are removable, then `list.AddRange(list)` doubles what it has before appending `PaelsGrowth` — a weighting trick that halves Growth's odds. Their relics need modelling too. |
