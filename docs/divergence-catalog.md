...
Three, all past the act 1 boss — territory nothing could reach until buffed runs started
winning.

| # | Metric | Seed | What is known |
| --- | --- | --- | --- |
| O10 | `state_type` step 325: game `map` (act 2), emulator `game_over` | `3PFLW9XC5D-a8-buff400` | **There is no act transition.** `AdvanceAfterRelicReward` treats a boss node as the end of the run. The act 1 boss dying should open act 2: a new map, a new encounter and event sequence, and whatever the game does at the boundary. This is a feature, not a fix — the emulator has `ActUnderdocks` and `act_N_map` streams, so the machinery is partly there, but nothing about act 2 has ever been compared against the game. |
...
| O12 | (act 2) nothing past the transition is compared at all | `3PFLW9XC5D-a8-buff400`, `7WGQ2VNJ4M-a8-buff400` | Both buffed runs died on act 2 floors 21-22, so ~140 steps of act 2 are captured and waiting behind O10. |
