...
- Native relic combat effects for 36 relics, with run-level HP, potions, and relics passed into native combat. The set is picked for coverage of the hooks a policy has to reason about rather than by act, since nearly every relic can appear in nearly any act: combat-start buffs (Vajra, Oddly Smooth Stone, Bronze Scales, Blood Vial, Bag of Preparation, Data Disk, Gorget, Anchor, Booming Conch), turn-scheduled effects (Lantern, Horn Cleat, Captain's Wheel, Happy Flower, Pendulum, Akabeko, Red Mask, Bag of Marbles, Festive Popper, Venerable Tea Set), per-card-played counters (Shuriken, Kunai, Ornamental Fan, Letter Opener, Nunchaku, Permafrost, Mummified Hand), damage-triggered relics (Centennial Puzzle, Self-Forming Clay), the +1 energy family and its downsides (Ectoplasm, Sozu, Velvet Choker, Spiked Gauntlets, Philosopher's Stone, Blessed Antler), and the HP-threshold and deck-editing ones (Red Skull, Orichalcum, Stone Cracker).
