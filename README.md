...
- Native relic effects for 56 relics — 47 with combat effects and 9 that act only between combats — with run-level HP, potions, and relics passed into native combat. The set is picked for coverage of the hooks a policy has to reason about rather than by act, since nearly every relic can appear in nearly any act: combat-start buffs (Vajra, Oddly Smooth Stone, Bronze Scales, Blood Vial, Bag of Preparation, Data Disk, Gorget, Anchor, Booming Conch), turn-scheduled effects (Lantern, Horn Cleat, Captain's Wheel, Happy Flower, Pendulum, Akabeko, Red Mask, Bag of Marbles, Festive Popper, Venerable Tea Set), per-card-played counters (Shuriken, Kunai, Ornamental Fan, Letter Opener, Nunchaku, Kusarigama, Tuning Fork, Ivory Tile, Permafrost, Mummified Hand), damage-triggered relics (Centennial Puzzle, Self-Forming Clay), relics that read the previous turn (Art of War, Pocketwatch), turn-end relics (Cloak Clasp, Screaming Flagon, Stone Calendar, Parrying Shield), between-combat heals (Meal Ticket, Regal Pillow), the +1 energy family and its downsides (Ectoplasm, Sozu, Velvet Choker, Spiked Gauntlets, Philosopher's Stone, Blessed Antler), and the HP-threshold and deck-editing ones (Red Skull, Orichalcum, Stone Cracker). The run-level set covers the deck (War Paint, Whetstone, Frozen/Molten/Toxic Egg), the reward screen (White Beast Statue), the shop (Membership Card), the rest site (Tiny Mailbox) and death (Lizard Tail).
