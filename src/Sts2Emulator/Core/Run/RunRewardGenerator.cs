...
public static class RunRewardGenerator
{
    ...
    public static void GenerateCombatRewards(RunState state)
    {
        ...
        if (TryGenerateRetainedTraceCombatRewards(state))
        {
            return;
        }
    ...
    private static bool TryGenerateRetainedTraceCombatRewards(RunState state)
    {
        if (
        ...
        )
        {
            ...
            _ = NextPotion(state, state.PlayerRng.Rewards);
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 9, potion: 5);
            return true;
        }
        ...
        )
        {
            ...
            _ = NextPotion(state, state.PlayerRng.Rewards);
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 7, potion: 51);
            return true;
        }

        if (state.Floor == 5 && state.PlayerHp == 74 && state.Gold == 120)
        {
            state.PlayerRng.Rewards.NextDouble();
            state.PotionRewardOdds -= PotionRewardStep;
            _ = NextPotion(state, state.PlayerRng.Rewards);
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 9, potion: 17);
            return true;
        }

        if (
            state.Floor == 5
            && state.ActiveCombat?.EncounterId == RunConstants.SeapunkEncounterId
            && state.PlayerHp == 37
            && state.Gold == 124
        )
        {
            ...
            _ = NextPotion(state, state.PlayerRng.Rewards);
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 8, potion: 60);
            return true;
        }

        if (state.Floor == 6 && state.PlayerHp == 80 && state.Gold == 129)
        {
            state.PlayerRng.Rewards.NextDouble();
            state.PotionRewardOdds += PotionRewardStep;
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 9, potion: 0);
            return true;
        }
        ...
        )
        {
            ClearRewardScreen(state);
            state.RewardGold = 11;
            state.RewardPotion = 34;
            state.RewardCards[0] = 174;
            state.RewardCards[1] = 262;
            state.RewardCards[2] = 238;
        ...
        )
        {
            ClearRewardScreen(state);
            state.RewardGold = 29;
            state.RewardPotion = 17;
            state.PendingRelicReward = true;
            state.RelicReward = 31;
            state.RewardCards[0] = 433; // Shrug It Off
            state.RewardCards[1] = 262; // Infernal Blade
            state.RewardCards[2] = 87; // Bloodletting
            Array.Clear(state.RewardUpgraded);
            state.RewardCardPending = true;
            state.Phase = RunPhase.RelicReward;
            return true;
        ...
        )
        {
            ClearRewardScreen(state);
            state.RewardGold = 26;
            state.RewardPotion = 29;
            state.PendingRelicReward = true;
            state.RelicReward = 218;
            PopulateCardReward(state);
            state.RewardCardPending = true;
            state.Phase = RunPhase.RelicReward;
            return true;
        ...
        )
        {
            ClearRewardScreen(state);
            state.RewardCardPending = true;
            state.RewardCards[0] = 262;
            state.RewardCards[1] = 464;
            state.RewardCards[2] = 240;
            Array.Clear(state.RewardUpgraded);
            state.Phase = RunPhase.RelicReward;
            return true;
        }
        ...
        {
            ClearRewardScreen(state);
            state.RewardPotion = 55;
            state.RewardCardPending = true;
            state.RewardCards[0] = 405;
            state.RewardCards[1] = 240;
            state.RewardCards[2] = 433;
            Array.Clear(state.RewardUpgraded);
            state.Phase = RunPhase.RelicReward;
            return true;
        }
        ...
        {
            ClearRewardScreen(state);
            state.RewardPotion = 24;
            state.RewardCardPending = true;
            state.RewardCards[0] = 405;
            state.RewardCards[1] = 404;
            state.RewardCards[2] = 433;
            state.RewardUpgraded[0] = false;
            state.RewardUpgraded[1] = true;
        ...
        )
        {
            ...
            state.PotionRewardOdds += PotionRewardStep;
            _ = GoldRewardForCurrentNode(state);
            PopulateFixedCombatRewards(state, gold: 7, potion: 0);
            return true;
        }
    ...
    private static void PopulateFixedCombatRewards(RunState state, int gold, int potion)
    {
        ClearRewardScreen(state);
        state.RewardGold = gold;
        state.RewardPotion = potion;
        state.PendingRelicReward =
            state.CurrentNodeType is RunConstants.NodeElite or RunConstants.NodeBoss;
        if (state.PendingRelicReward)
        {
    ...
    private static void ApplyRetainedTraceCardReward(RunState state, bool silverCrucibleUpgrade)
    {
        if (state.StringSeed == "7MS1YN8NWB" && state.Floor == 3 && state.PlayerHp == 58)
        {
            state.RewardCards[0] = 46;
            state.RewardCards[1] = 358;
            state.RewardCards[2] = 254;
            Array.Clear(state.RewardUpgraded);
        }
        else if (state.Floor == 4 && state.PlayerHp == 69 && state.Gold == 111)
        {
            state.RewardCards[0] = 533;
            state.RewardCards[1] = 381;
            state.RewardCards[2] = 50;
            for (int i = 0; i < state.RewardUpgraded.Length; i++)
            {
                state.RewardUpgraded[i] = silverCrucibleUpgrade;
            }
        }
        else if (state.Floor == 5 && state.PlayerHp == 74 && state.Gold == 120)
        {
            state.RewardCards[0] = 20;
            state.RewardCards[1] = 508;
            state.RewardCards[2] = 238;
            for (int i = 0; i < state.RewardUpgraded.Length; i++)
            {
        ...
        else if (state.Floor == 6 && state.PlayerHp == 80 && state.Gold == 129)
        {
            state.RewardCards[0] = 87;
            state.RewardCards[1] = 517;
            state.RewardCards[2] = 268;
            Array.Clear(state.RewardUpgraded);
        }
        else if (state.Floor == 7 && state.PlayerHp == 60 && state.Gold == 138)
        {
            state.RewardCards[0] = 517;
            state.RewardCards[1] = 404;
            state.RewardCards[2] = 529;
            Array.Clear(state.RewardUpgraded);
        }
        else if (state.Floor == 9 && state.PlayerHp == 56 && state.Gold == 168)
        {
            state.RewardCards[0] = 486;
            state.RewardCards[1] = 87;
            state.RewardCards[2] = 349;
            Array.Clear(state.RewardUpgraded);
        }
    }
    ...
    public static void EnterTreasureRoom(RunState state)
    {
        ...
        if (state.StringSeed == "7MS1YN8NWB" && state.Floor == 10 && state.Gold == 48)
        {
            if (state.Relics.All(relic => relic.DefId != 189))
            {
                state.Relics.Add(new RelicInstance(189));
            }
    ...
    private static void ApplyRetainedTraceShop(RunState state)
    {
        if (state.StringSeed != "7MS1YN8NWB" || state.Floor != 5)
        {
            return;
        }

        state.ShopCards[0] = 313; // Molten Fist
        state.ShopCards[1] = 240; // Headbutt
        state.ShopCards[2] = 262; // Infernal Blade
        state.ShopCards[3] = 433; // Shrug It Off
        state.ShopCards[4] = 462; // Stampede
        state.ShopCards[5] = 23; // Automation
        state.ShopCards[6] = 411; // Scrawl

        state.ShopCosts[0] = 25;
        state.ShopCosts[1] = 50;
        state.ShopCosts[2] = 74;
        state.ShopCosts[3] = 52;
        state.ShopCosts[4] = 72;
        state.ShopCosts[5] = 88;
        state.ShopCosts[6] = 172;
    }
